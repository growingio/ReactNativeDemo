...
export default class CstmEventTest extends Component {
    render() {
        //Cstm事件
        var data = [];
        data.push({key:"1",oper:"track('GIOKey')"});
        data.push({ key:"2",oper: "track('')" })
        data.push({ key: "3",oper: "track(null)" })
        ...
        data.push({ key: "22", oper: "" })
        data.push({ key: "23", oper: "" })
        // for (var i = 0; i < 100; i++) {
    ...
    _header = () => {
        return <View style={styles.ViewForTextStyle}><Text style={[styles.txt, { backgroundColor: 'orange'}]}>Cstm事件测试</Text></View>;
    }
    ...
    _separator = () => {
        return <View style={{ height: 2, backgroundColor: '#ececec' }} />;
    }
    ...
    clickItem(item) {
        ...
        {
            case "1":
                console.log("track('GIOKey')");
                NativeModules.GrowingIO.track("GIOKey",null);
               break;
            case "2":
                console.log("track('')");
